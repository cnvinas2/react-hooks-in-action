  ...
  return useQuery(
    ...
    () => getData(`http://localhost:3001/bookables/${id}`),
    {
      ...
      initialData: queryClient
        ...
        ?.find((b: any) => b.id === parseInt(id, 10))
    }
  );
}
...
function useUpdateBookable () {
  ...
  const mutation = useMutation(
    (item: any) => editItem(`http://localhost:3001/bookables/${item.id}`, item),
    {
      onSuccess: bookable => {
 ...
 */
function updateBookablesCache (bookable: any, queryClient: any) {
  ...
  const bookableIndex = bookables.findIndex((b: any) => b.id === bookable.id);
...
function useDeleteBookable () {
  ...
  const queryClient = useQueryClient();
  const mutation = useMutation(
    (bookable: any )=> deleteItem(`http://localhost:3001/bookables/${bookable.id}`),
    {
      /* on success receives the original item as a second argument */
      onSuccess: (response, bookable: any) => {
        // get all the bookables from the cache
        const bookables: any = queryClient.getQueryData("bookables") || [];

        // set the bookables cache without the deleted one
        queryClient.setQueryData(
          "bookables",
          bookables.filter((b: any) => b.id !== bookable.id)
        );
...
function getIdForFirstInGroup (bookables: any, excludedBookable: any) {
  ...
  const bookableInGroup = bookables.find((b: any) => b.group === group && b.id !== id);
